...
HRESULT Renderer::InitPipeline()
{
	...
	//D3DCompileFromFile is the function that handles the loading and compilation of the shader.
	...
	g_dev->CreateVertexShader(VS->GetBufferPointer(), VS->GetBufferSize(), NULL, &pVS);
	g_dev->CreatePixelShader(PS->GetBufferPointer(), PS->GetBufferSize(), NULL, &pPS);
	...
	result = g_dev->CreateInputLayout(ied, ARRAYSIZE(ied), VS->GetBufferPointer(), VS->GetBufferSize(), &pLayout);
	VS->Release();
	PS->Release();
	if (FAILED(result))
	{
...
void Renderer::InitGraphics()
{
	...
	bd.ByteWidth = sizeof(Vertex) * 3; //size of the buffer in bytes - size of the vertex struct * 3 vertices
