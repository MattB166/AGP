...
Vout main(Vin Input)
{
    ...
    float3 wvpos = mul(WV, float4(Input.pos, 1.0f)).xyz;
    float3 wvnormal = mul(WV, float4(Input.normal, 0)).xyz;
    float3 eyer = -normalize(wvpos);
    output.reflectionUVW = 2.0 * dot(eyer, wvnormal) * wvnormal - eyer;
    output.reflectionUVW = mul(float4(output.reflectionUVW, 0), WV).xyz;
